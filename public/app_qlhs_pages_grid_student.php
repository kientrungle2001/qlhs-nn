<div>
	<easyui.layout.panel collapsible="true" title="Thêm học sinh" collapsed="true"> <br />
		<hform id="add-dg" method="post" title="Thêm học sinh mới">
			...
			<strong> Ngày nhập học: </strong><form.textField type="date" name="startStudyDate" value="<?php echo date('Y-m-d'); ?>" />
			<strong> </strong>
	...
	<dg.dataGrid id="dg" title="Quản lý học sinh" scriptable="true" layout="easyui/datagrid/datagrid" 
		...
		<layout.toolbar id="dg_toolbar">
			...
			sql="select id as value, 
				...
				<layout.toolbarItem action="$dg.detail({url: '<?php echo BASE_REQUEST . '/student/detail'; ?>', 'gridField': 'id', 'action': 'render', 'renderRegion': '#student-detail'});" icon="sum" />
			</hform>
		</layout.toolbar>
	...
	<div style="float:left; margin-left: 20px; margin-top: 20px; width: auto;">
		...
		<layout.toolbarItem action="$dg.addToTable({url: '<?php echo BASE_REQUEST . '/dtable/add'; ?>?table=class_student', 'gridField': 'studentId', 'tableField': 'classId', 'tableFieldSource': '#cmbClass', 'tableField2': 'startClassDate', 'tableFieldSource2': '#startStudyDate4'})" icon="add" />
		<br />
		...
		<layout.toolbarItem action="$dg.addToTable({url: '<?php echo BASE_REQUEST . '/dtable/add'; ?>?table=class_student', 'gridField': 'studentId', 'tableField': 'classId', 'tableFieldSource': '#cmbClass2', 'tableField2': 'startClassDate', 'tableFieldSource2': '#startStudyDate'}); $dg.addToTable({url: '<?php echo BASE_REQUEST . '/dtable/update'; ?>?table=class_student', 'gridField': 'studentId', 'tableField': 'classId', 'tableFieldSource': '#cmbClass3', 'tableField2': 'endClassDate', 'tableFieldSource2': '#startStudyDate'})" icon="add" />
		<br />
		...
		<layout.toolbarItem action="$dg.addToTable({url: '<?php echo BASE_REQUEST . '/dtable/update'; ?>?table=class_student', 'gridField': 'studentId', 'tableField': 'classId', 'tableFieldSource': '#cmbClass4', 'tableField2': 'endClassDate', 'tableFieldSource2': '#startStudyDate3'}); $dg.reload();" icon="add" />
		</easyui.layout.panel>

...
class PzkNewsComments extends PzkObject
{
	...
	{
		$allcomments=_db()->useCB()->select("comment.*, user.avatar,user.name ")->from("comment")->join('user', 'comment.userId=user.id')->where(array('newsId',$newsid))->limit(5)->orderBy('id desc')->result();
		return($allcomments);
	}

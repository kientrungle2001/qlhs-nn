...
<?php
	...
	<table border="1" cellpadding="4px" cellspacing="0" style="border-collapse:collapse;margin: 15px;width: 1000px;">
			...
			<?php 
				if(isset($orders[$class['id']][$student['id']][0])) {
					...
					?>
						...
						<td style="text-align: right;">
						<span title="<?php echo @$order['amount'];?>đ">Đã thanh toán</span>
						</td>
				<?php 
					}
				} else { if(0) { ?>
					<a href="javascript:void(0)" onclick="makeOrder('<?php echo @$class['id'];?>', '<?php echo @$student['id'];?>', '0', '<?php echo ((@$summary[$class['id']][0] - $lastDiscount) * $class['amount']); ?>')"> Đóng tiền</a>
				<?php } ?>
		...
		if ($teacher) { 
			...
			GV <strong><?php echo @$teacher['name'];?></strong> dạy: <?php echo pzk_or($teacherTotal, '0 ') ?> buổi<br />
		<?php } ?>
		...
		if ($teacher2) { 
			...
			GV <strong><?php echo @$teacher2['name'];?></strong> dạy: <?php echo pzk_or($teacher2Total, '0 ') ?> buổi<br />
		<?php } ?>
		<?php
		...
		<?php if ($teacher && $teacherAmount) { ?>
			Lương GV <strong><?php echo @$teacher['name'];?></strong>: <?php echo product_price(pzk_or($teacherAmount, '0')) ?> (<?php echo @$teacher['salary'];?>%)<br />
		<?php } ?>
		<?php if ($teacher2 && $teacher2Amount) { ?>
			Lương GV <strong><?php echo @$teacher2['name'];?></strong>: <?php echo product_price(pzk_or($teacher2Amount, '0')) ?> (<?php echo @$teacher2['salary'];?>%)<br />
		<?php } ?>
	...
	<table border="1" cellpadding="4px" cellspacing="0" style="border-collapse:collapse;margin: 15px;width: 1000px;">
		<tr>
			<th colspan="9"><?php echo @$period['name'];?> (<?php echo pzk_or(@$summary[$class['id']][$period['id']], '<span>0</span>'); ?> buổi)</th>
		</tr>
		...
		<tr>
			...
			<td style="text-align: right;"><?php echo @$schedules[$class['id']][$student['id']][$period['id']]['status']['5']?>&nbsp;</td>
				...
				<?php 
					...
					<a href="javascript:void(0)" onclick="makeOrder('<?php echo @$class['id'];?>', '<?php echo @$student['id'];?>', '<?php echo @$period['id'];?>', '<?php echo ((@$summary[$class['id']][$period['id']] - $lastDiscount) * $class['amount']); ?>')"> Đóng tiền</a>
				<?php } ?>

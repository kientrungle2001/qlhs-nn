...
<?php foreach ( $sets as $s ) : ?>
	<a href="<?php echo BASE_REQUEST . '/attribute'; ?>/testSet?id=<?php echo @$s['id'];?>"><?php echo @$s['title'];?></a> |
<?php endforeach; ?>
<dg.dataGrid id="dg" controller="/dset" title="<?php echo $set->get('title')?>" scriptable="true" 
	table="<?php echo $set->get('code')?>" width="800px" height="450px" fckeditors="<?php echo @$fckeditors;?>" multiselects="<?php echo @$comboboxs;?>">
	...
	<layout.toolbar id="dg_toolbar">
		<hform id="dg_search" onsubmit="pzk.elements.dg.search({'fields': {<?php echo @$searchFields;?>}}); return false;">
		<table>
		<?php 
		$filterCount = 0;
		foreach($attributes as $attribute) {
...
<form.combobox id="cmbGroup" name="groupId"
	sql="select id as value, 
		...
		layout="category-select-list" />
<layout.toolbarItem action="$dg.addToTable({url: '<?php echo BASE_REQUEST . '/dset/add'; ?>?table=attribute_group_attribute', 'gridField': 'attributeId', 'tableField': 'setId', 'tableFieldSource': '#cmbSet', 'tableField2': 'groupId', 'tableFieldSource2': '#cmbGroup'})" icon="add" />
<?php } ?>
</div>
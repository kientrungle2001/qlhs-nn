...
<div class="order_wrapper">
	<div class="order_header">
		...
		<div class="order_date">
			<div class="order_line">
				...
				<span class="order_line_value">Ngày <?php echo date('d/m/Y', strtotime($order['created'])) ?></span>
			</div>
		</div>
		<div class="order_no">
		Quyển số: <?php echo @$order['bookNum'];?><br />
		Số: <?php echo @$order['noNum'];?><br />
		Nợ: <?php echo @$order['debit'];?><br />
		Có: <?php echo @$order['balance'];?>
		</div>
	...
	<div class="order_body">
		<div class="order_line">
			...
			<span class="order_line_value"><?php echo @$order['name'];?></span>
		</div>
		<div class="order_line">
			<span class="order_line_label">Số điện thoại: </span>
			<span class="order_line_value"><?php echo @$order['phone'];?></span>
		</div>
		<div class="order_line">
			<span class="order_line_label">Địa chỉ: </span>
			<span class="order_line_value"><?php echo @$order['address'];?></span>
		</div>
		<div class="order_line">
			<span class="order_line_label">Lý do thu: </span>
			<span class="order_line_value"><?php echo @$order['reason'];?></span>
		</div>
		<div class="order_line">
			<table border="1" style="border-collapse: collapse; width: 100%">
				...
				<tr>
					...
					<td><?php echo $order_item['muster'] ?></td>
					<td><?php echo product_price($order_item['price']) ?></td>
					<td><?php echo product_price($order_item['total_before_discount']) ?></td>
					<td><?php echo product_price($order_item['discount']) ?></td>
					<td><?php echo $order_item['discount_reason'] ?></td>
					<td><?php echo product_price($order_item['amount']) ?></td>
				</tr>
				<?php } ?>
			</table>
		</div>
		<div class="order_line">
			...
			<span class="order_line_value"><?php echo product_price($order['amount']); ?></span>
		</div>
		<div class="order_line">
			<span class="order_line_label">Kèm theo: </span>
			<span class="order_line_value"><?php echo @$order['additional'];?></span>
			<span class="order_line_label">Chứng từ kế toán: </span>
			<span class="order_line_value"><?php echo @$order['invoiceNum'];?></span>
		</div>
	</div>
	<br />
